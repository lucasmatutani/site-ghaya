...
$imovel_destaque = $_REQUEST['imovel_destaque'];
$zap = $_REQUEST['zap'];
...
//salvar imagem no banco e no servidor
if (isset($_FILES['images']['name']) && isset($_POST['destaque'])) {
    ...
    $destaque = $_POST['destaque'];  // Aqui, assumimos que 'destaque' é um array
    ...
    foreach ($_FILES['images']['name'] as $key => $name) {
        ...
        $isDestaque = $destaque[$key];  // Obter o valor de 'destaque' para essa imagem

        if (move_uploaded_file($_FILES['images']['tmp_name'][$key], $destination)) {
            $sql = "INSERT INTO images (image_path, imovel_id, destaque) VALUES ('https://ghayaimoveis.com/{$path_to_img}', '{$codigo_interno}', '{$isDestaque}')";
            $conn->query($sql);
        }
    }
} else {
...
if ($zap == 1) {
    ...
    $data = $conn->query("SELECT * FROM images WHERE imovel_id = '$codigo_interno'");
    $images = [];
    if ($data->num_rows > 0) {
